...
def get_dir(xdg_name):
    ...
    elif xdg_name in __default_user_dirs:
        directory = os.path.expandvars(__default_user_dirs[xdg_name])
    else:
        return None
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="displays current XDG user " +
                                     "directories.")
    parser.add_argument("name", choices=__default_user_dirs.keys(),
                        help="variable name corresponding to a XDG " +
                        "user directory")
    args = parser.parse_args()
    print(get_dir(args.name))
